...
void eraseHistoryLog();
//erase HistoryLog.txt
...
void ballvalues(vector<int>& urn, int& ballvalues, ofstream& fout)
{
    int ball[3]={0};
    int ballpicked=0;
    for(int i=0;i<3;i++)
...
void eraseHistoryLog()
{
    ...
    //open file
    fout.open("HistoryLog.txt",std::ios::out | std::ios::trunc);
    ...
    fout<< "History log for each customer's discount"<<endl;
