...
void Output(int x,float time,float speed);
...
int main()
{
  ...
  {
      List(trips);
      cout<<YEL<<"Please input the number of the city ("<<GRN<<"0-9"<<YEL<<"): "<<GRN;
      cin>>x;
      cout<<RESET;      
      check(x);
      Output(x,speed,time);
...
{ 
  while(x>9||x<0)
  {
    cout<<RED<<"Invalid input, please re-input the number("<<GRN<<"0-9"<<") : ";
    cout<<GRN;
    cin>>x;
    cout<<RESET;
    check(x);
...
{    
     trips[x].Output();
     speed=trips[x].getSpeed();
     time=trips[x].getTime(speed);
     timer(time,speed);
}
