...
void Read(foodptr &head,foodptr &tail);
...
int findcal(foodptr head,float key,foodptr tmp);

//delete the selected node from the linked list (choice 4)
void delnode(foodptr &head,int node,foodptr tmp);

//delete the selected node from the linked list (choice 4)
void filter(foodptr &head,int key);
