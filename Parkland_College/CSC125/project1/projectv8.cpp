...
int main()
{
    ...
    do
    {
	    ...
	    //loop to find the node which is bigger than limitation
	    //delete the node in the linked list
            while(tmp3!=NULL)
	    {
	        //get the number of the node which is bigger than limitation
                node=findcal(head,filter);

                //debug
		//cout<<node<<endl;

                    //delete the node which is bigger than limitation in the linked list
                    delnode(head,node);
...
int findcal(foodptr head,float filter)
{
    ...
    tmp5=head;
    int i=1;
    while(tmp5->cal<=filter&&tmp5->next!=NULL)
    {
        tmp5=tmp5->next;
        i++;
    }
